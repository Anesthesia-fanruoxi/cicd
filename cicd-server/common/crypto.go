...
func DecryptAndDecompress(data string) ([]byte, error) {
	...
	if err != nil {
		Logger.Errorf("Base64解码失败: %v", err)
	...
	if err != nil {
		Logger.Errorf("创建gzip reader失败: %v", err)
...
func CompressAndEncrypt(data []byte) (string, error) {
	...
	if err := gzipWriter.Close(); err != nil {
		Logger.Errorf("关闭gzip写入器失败: %v", err)
	...
	if err != nil {
		Logger.Errorf("创建AES加密器失败: %v", err)
